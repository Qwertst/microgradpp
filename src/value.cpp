...
namespace nn {
    ...
    Value pow(const Value& arg, double k) {
        ...
        res->backward_ = [weak_res, k]() {
            weak_res->prev_[0]->grad_ += k * std::pow(weak_res->prev_[0]->data_, k-1)*weak_res->grad_;
        };
        return res;
    }
    ...
    Value relu(const Value& arg) {
        ...
        Value_handler* weak_res = res.get();
        res->backward_ = [weak_res]() {
            weak_res->prev_[0]->grad_ += (weak_res->data_ > 0 ? 1 : 0)*weak_res->grad_;
        };
        return res;
    }    
    ...
    void Value_handler::top_sort(const Value& value, std::unordered_set<Value>& visited, std::vector<Value>& order) {
