...
namespace nn {
    ...
    Neuron::Neuron(size_t in_size, bool bias, bool nonlin): in_size_(in_size), nonlin_(nonlin) {
        if (bias) {
            bias_ = make_value();
        }
    ...
    Value Neuron::operator()(const std::vector<Value>& input) const {
        Value res = bias_;
        std::for_each(weights_.begin(), weights_.end(),[&res](const Value& w){
            res = res + w;
        });
        return nonlin_ ? relu(res) : res;
    }

    Layer::Layer(size_t in_size, size_t out_size, bool bias, bool nonlin): in_size_(in_size), out_size_(out_size) {
    ...
    MLP::MLP(size_t in_size, std::vector<size_t> out_sizes): in_size_(in_size), out_sizes_(std::move(out_sizes)), n_layers_(out_sizes_.size()) {
        layers_.emplace_back(in_size, out_sizes_[0]);
        for (size_t it = 1; it < n_layers_; ++it) {
            layers_.emplace_back(out_sizes_[it-1], out_sizes_[it], true, it != n_layers_-1);
        }
    }
